...
import {
  createNewSurveyMessage,
  createSurveyClosedMessage,
  createSurveyClosingSoonDM,
  createSurveyClosedDM,
} from "../templates/messageTemplates";
...
const normalizeSurveyData = (survey: any) => {
  return {
    ...survey,
    closeAt:
      survey.closeAt || survey.settings?.closeTime || survey.closedAt || null,
...
export const notifyNewSurvey = async (
  survey: any,
  channelIds: string[],
  ...
  try {
    ...
    // Lấy thông tin survey với questions để kiểm tra matrix
    ...
    const hasMatrixQuestions =
      surveyWithQuestions?.questions?.some(
    ...
    const message = createNewSurveyMessage({
      ...
      createdBy: surveyWithQuestions?.user?.email || "Unknown User",
...
export const notifySurveyClosingSoon = async (survey: any) => {
  try {
    console.log(
      "Sending survey closing soon notification for survey:",
      survey.id
    );
...
export const setupChannelSelection = () => {
  slackApp.action(
    "select_channel",
    async ({ ack, body, client }: { ack: any; body: any; client: any }) => {
      await ack();
      ...
      try {
        ...
        const channels: PlainTextOption[] =
